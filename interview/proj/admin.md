...
use-context：类型安全的上下文钩子
    ...
    实现页面/组件级别的全局数据
    页面/组件销毁后，数据一并销毁
use-async-watch: 解决竞态问题的异步 watch
...
### 优化
sourcemap：生产环境开启以辅助问题定位
    个人项目：部署 map 文件 + Nginx IP 白名单限制访问
    商业项目：构建时通过 @sentry/vite-plugin 自动上传到 Sentry → 删除本地 map → 仅部署 JS → 生产报错时在监控平台还原源码位置

gzip 压缩：通过压缩文本资源减小传输体积，以极小的 CPU 开销换取显著的网络传输时间节省

http 缓存：
    静态资源：强缓存
    html 文件
        html 配 hash：会导致访问入口不稳定，不断更新 nginx 配置
        协商缓存：收益不大，单页应用 html 文件小，ssr 可采用
        不缓存：常用方案
    动态接口响应：读多写少 + 响应体大 → 协商缓存
        根据 updatedAt + id + version 组合生成 etag
